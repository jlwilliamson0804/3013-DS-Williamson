...
int main()
{
	...
	vector<int> V;
	int n = 0; //data value being read in from file
	int temp = 0;
	ifstream infile;
	infile.open("data.txt"); // input file name and opens it

	infile >> n; // begins the loop by getting first value
	while (!infile.eof()) // retrieve all values in the file
	{
		V.push_back(n);
		infile >> n;
	}
